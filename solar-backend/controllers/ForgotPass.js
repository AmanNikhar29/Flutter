...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    const [user] = await db.execute('SELECT * FROM Seller WHERE email = ?', [email]);
    if (user.length === 0) {
    ...
    await db.execute(
      'UPDATE Seller SET otp = ?, otp_expiration = ? WHERE email = ?',
...
const verifyOTP = async (req, res) => {
  ...
  try {
    ...
    const [user] = await db.execute(
      'SELECT * FROM Seller WHERE email = ? AND otp = ? AND otp_expiration > NOW()',
      [email, otp]
    );
